...
model=${AI_WORKER_MODEL:-opencode-go/glm-5.2:high}
herdr agent start worker --cwd "$worker_path" --workspace "$workspace_id" --no-focus -- \
  ai-run "$profile" -- pi --model "$model"
```

Use unique worker names when another worker exists. Follow the `herdr` skill for prompt submission,
state waits, output reads, and cleanup.
